...
export default function PublishControls({ eventId, publications, selectedChannels }: Props) {
  ...
  function updateChannel(channel: string, updates: Partial<Publication>) {
    setRows((current) => current.some((item) => item.channel === channel)
      ? current.map((item) => item.channel === channel ? { ...item, ...updates } : item)
      : [...current, { channel, enabled: true, status: "pending", external_url: null, last_error: null, ...updates }]);
  }
  ...
  async function runChannel(channel: WritableChannel, action: "create" | "update" | "delete" = "create", ask = true) {
    ...
    setPublishing(`${channel}:${actualAction}`); setMessage(""); setError("");
    if (actualAction !== "delete") updateChannel(channel, { status: "publishing", last_error: null });
    try {
      const response = await fetch(`/api/publish/${channel}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ eventId, action: actualAction }) });
      ...
      if (actualAction === "delete") {
        ...
        setMessage(`${label} item deleted. The Event Hub record was kept.`);
      } else {
      ...
      updateChannel(channel, { status: "failed", last_error: text }); setError(text); return false;
        ...
        {channels.map((channel) => {
          ...
          return (
            <div className="publishChannel" key={channel.id}>
              <div className="publishRow"><div><strong>{channel.label}</strong><small>{enabled ? "Selected" : "Not selected"}</small></div><span className={`statusPill status-${status}`}>{status.replaceAll("_", " ")}</span></div>
